...
- Read **[docs/yaml-catalog.md](docs/yaml-catalog.md)** — authoritative schema reference, including `io` / `verify` / `on_verify_fail`, and stage `mcp` / `.mcp.json` ([Stage MCP](docs/yaml-catalog.md#stage-mcp)). Do not author `clonable` / `clone_forks` (rejected; see [Clone Chain spec](docs/specs/clone-chain.md)).
