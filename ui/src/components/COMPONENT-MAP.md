...
| PipelineTrack | `components/PipelineTrack.tsx` | wired | Horizontal stage nodes + envelope chips. Run mode on `RunDetailPage`; `mode="definition"` on pipeline detail (lines only). |
| RunTrack | `components/RunTrack.tsx` | wired | Facade for run-detail track band: linear `PipelineTrack` or DAG `PipelineDagTrack` plus `TrackDetailList`. Used on `RunDetailPage`. |
| PipelineDagTrack | `components/PipelineDagTrack.tsx` | wired | Horizontal layered mini-DAG: columns per `layer`, gutter wires between layers, reuses `StageNode` + `TrackWire`. |
| TrackDetailList | `components/TrackDetailList.tsx` | wired | Dependency-aware stage rows under track summary: status, attempt badge, readiness, gate chips, multi-wait emphasis, **Retry stage** on failed rows, **Abandon** on running rows. Row click selects stage. |
...
| useStageRetry | `stageRetry/useStageRetry.ts` | wired | In-flight stage retry state, error handling, post-success refresh callback, and retry affordance when stage status is `failed`. |
| useStageAbandon | `stageAbandon/useStageAbandon.ts` | wired | In-flight stage abandon state, confirm dialog, error handling, post-success refresh callback, and abandon affordance when stage status is `running`. |
| StageNode | `components/StageNode.tsx` | wired | Single pipeline node with status ring, label, optional meta. Pending omits `onClick`. |
| StageWorkspace | `workspace/resolveRunWorkspace.ts` | wired | Pure resolver returns selected stage, workspace kind, composer, decide, drawer, `trackLayout`, `detailListRows`, and track. `RunDetailPage` renders it. |
| TranscriptStream | `components/TranscriptStream.tsx` | wired | Scrollable transcript chrome (head + body + pinned composer). Auto-scrolls while running or waiting. Session alive/closed chip is composed in `RunDetailPage`. |
...
| EnvelopeFields / EnvelopeRecord | `components/EnvelopeFields.tsx` | wired | Shared `.drawer__grid` + `.kv` body; full record is a run-shell workspace (`#/runs/:id/stages/:stageId/envelope`). |
| ArtifactReader | `components/ArtifactReader.tsx` | wired | Center-column markdown/code viewer. Rendered/Raw; Diff disabled. Hash `#/runs/:id/artifacts?path=`. |
