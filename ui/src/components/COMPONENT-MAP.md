...
| Settings page | `pages/SettingsPage.tsx` | wired | Appearance, Providers (API keys + OAuth + credential source), project catalog MCP inspect, MCP connection instructions, session-slot select via `POST /api/settings`, held-stage copy, notify preference. First-run/connect gate at `#/connect`. |
