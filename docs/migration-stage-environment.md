...
1. **Credentials** — add `secrets:` on the stage (or pipeline entry). GitHub tokens default to `GIT_ASKPASS` + a materialised file (not raw env). Use `{ name: GITHUB_TOKEN, as: env }` when a tool requires the env var (emits a WARN); askpass remains set so plain HTTPS `git` still authenticates via the helper (not URL-embedded tokens).
