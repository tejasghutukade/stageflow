...
SQLite is the active `RunStore` adapter. Run state is **global**, not per-project: it lives under the durable root (`$STAGEFLOW_HOME`, default `~/.stageflow/`), with per-run and per-attempt workspaces under `runs/`. One `RunStore` instance backs every project on the machine; each run still records the `project_root` it came from, so views can be scoped back to one project. Per-project settings remain at `<git-root>/.stageflow/settings.json`. Pipeline/task catalog resolution (`stageflow.yaml`, `pipelines/`, `tasks/`) stays project-local, resolved from the invoking git root. See [Data directory](data-directory.md).
